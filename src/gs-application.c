...
static const gchar *
gs_application_get_data_dir (void)
{
	const gchar *dir;

	dir = g_getenv ("UNITY_SOFTWARE_DATA_DIR");
	if (dir != NULL && dir[0] != '\0')
		return dir;

	dir = g_getenv ("GSETTINGS_SCHEMA_DIR");
	if (dir != NULL && dir[0] != '\0')
		return dir;

#ifdef UNITY_SOFTWARE_SRCDATADIR
	if (g_file_test (UNITY_SOFTWARE_SRCDATADIR, G_FILE_TEST_IS_DIR))
		return UNITY_SOFTWARE_SRCDATADIR;
#endif
...
static void
gs_application_add_plugin_locations_from_root (GsApplication *app,
					       const gchar *root)
{
	GDir *dir;
	const gchar *entry;
	gboolean has_modules = FALSE;
	...
	if (dir == NULL) {
		g_debug ("Skipping plugin directory %s: unable to open", root);
		return;
	}
	...
	while ((entry = g_dir_read_name (dir)) != NULL) {
		...
		child = g_build_filename (root, entry, NULL);
		if (g_str_has_suffix (entry, ".so")) {
			has_modules = TRUE;
		} else if (g_file_test (child, G_FILE_TEST_IS_DIR)) {
			gs_application_add_plugin_locations_from_root (app, child);
		}
		g_free (child);
	}
	g_dir_close (dir);
	...
	if (has_modules)
		gs_plugin_loader_add_location (app->plugin_loader, root);
}

static void
gs_application_add_plugin_locations (GsApplication *app)
{
	const gchar *env_dirs = g_getenv ("UNITY_SOFTWARE_PLUGIN_DIRS");

	if (env_dirs != NULL && env_dirs[0] != '\0') {
		g_auto(GStrv) split = g_strsplit (env_dirs, G_SEARCHPATH_SEPARATOR_S, -1);
		for (guint i = 0; split[i] != NULL; i++) {
			if (split[i][0] == '\0')
				continue;
			gs_application_add_plugin_locations_from_root (app, split[i]);
		}
	}

#ifdef LOCALPLUGINDIR
	if (LOCALPLUGINDIR[0] != '\0')
		gs_application_add_plugin_locations_from_root (app, LOCALPLUGINDIR);
#endif
}
...
gs_application_initialize_plugins (GsApplication *app)
{
	...
	app->plugin_loader = gs_plugin_loader_new ();
		gs_application_add_plugin_locations (app);
...
gs_application_initialize_ui (GsApplication *app)
{
	...
	gtk_window_set_default_icon_name ("org.ubuntuunity.software");

	data_dir = gs_application_get_data_dir ();
	icon_theme = gtk_icon_theme_get_default ();
	if (data_dir != NULL && icon_theme != NULL) {
		g_autofree gchar *icon_search = g_build_filename (data_dir, "icons", "hicolor", NULL);
		if (g_file_test (icon_search, G_FILE_TEST_IS_DIR))
			gtk_icon_theme_append_search_path (icon_theme, icon_search);
	}

	if (icon_theme != NULL && !gtk_icon_theme_has_icon (icon_theme, "org.ubuntuunity.software")) {
		const gchar *fallback_dir = gs_application_get_data_dir ();
		if (fallback_dir != NULL) {
			g_autofree gchar *icon_path = g_build_filename (fallback_dir,
						       "icons",
						       "hicolor",
						       "scalable",
			...
			if (g_file_test (icon_path, G_FILE_TEST_EXISTS)) {
				...
				if (pixbuf != NULL) {
					GList *icons = NULL;
					icons = g_list_append (icons, g_object_ref (pixbuf));
					gtk_window_set_default_icon_list (icons);
					g_list_free (icons);
					g_object_unref (pixbuf);
				} else if (error != NULL) {
					g_debug ("Failed to register fallback icon: %s", error->message);
					g_clear_error (&error);
				}
			}
		}
		 ...
		 gpointer       user_data)
{
	...
	const gchar *copyright = "Copyright \xc2\xa9 2025 Gautham Nair";
	...
	gtk_about_dialog_set_logo_icon_name (dialog, "org.ubuntuunity.software");
...
_copy_file_to_cache (GFile *file_src, GError **error)
{
	...
	cache_dir = g_dir_make_tmp ("unity-software-XXXXXX", error);
	if (cache_dir == NULL)
		return NULL;
...
#endif
	settings = g_settings_new ("org.ubuntuunity.software");
...
gs_application_handle_local_options (GApplication *app, GVariantDict *options)
{
	...
	if (g_variant_dict_contains (options, "version")) {
		g_print ("unity-software " VERSION "\n");
		return 0;
	}
...
gs_application_new (void)
{
	return g_object_new (GS_APPLICATION_TYPE,
			     "application-id", "org.ubuntuunity.software",
