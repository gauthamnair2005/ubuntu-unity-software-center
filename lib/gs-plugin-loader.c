 ...
 *
 * Copyright (C) 2007-2018 Gautham Nair <gautham.nair.2005@gmail.com>
...
add_app_to_install_queue (GsPluginLoader *plugin_loader, GsApp *app)
{
	...
	g_source_set_name_by_id (id, "[unity-software] emit_pending_apps_idle");
				   ...
				   GsPluginLoader *plugin_loader)
{
	...
	/* plugin now allowing unity-software to show updates panel */
	if (allow_updates) {
					...
					GsPluginLoader *plugin_loader)
{
	...
	app = gs_plugin_loader_app_create (plugin_loader,
		"system/*/*/*/org.ubuntuunity.software.desktop/*");
	if (app != NULL)
		gs_plugin_event_set_app (event, app);
